...
/// counting resource wraps a memory resource to track allocations and count the current allocated size.
class counting_resource : public std::pmr::memory_resource {
public:
    ///
    /// \param underlying the underlying resource to use for allocation
    explicit counting_resource(std::pmr::memory_resource &underlying = *std::pmr::get_default_resource())

...
void Block::AddStmt(StmtPtr stmt) { _stmts.emplace_back(std::move(stmt)); }
...
std::shared_ptr<Assignment> Assignment::make(std::shared_ptr<Var> lhs,
                                             const ExprPtr &value) {
  return std::make_shared<Assignment>(std::move(lhs), value);
}

void Int::accept(Visitor *v) const { v->visit(this); }

std::shared_ptr<Int> Int::make(int32_t value) {
  return std::make_shared<Int>(value);
}

void Bool::accept(Visitor *v) const { v->visit(this); }
...
void BinOp::accept(Visitor *v) const { v->visit(this); }

...
void PrettyPrinter::print_indent() { stream << std::string(indent, ' '); }
...
void PrettyPrinter::visit(const ast::Int &op) { stream << op._value; }

void PrettyPrinter::visit(const ast::Name &op) { stream << op._name; }
...
void PrettyPrinter::visit(const ast::Record &op) {
  stream << "{\n";
  indent++;

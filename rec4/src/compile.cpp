...
void placeBB(InstructionList& instructions,
             std::map<BasicBlock*, int32_t>& locs,
             std::map<int32_t, BasicBlock*>& targets, BasicBlock* bb) {
  if (bb == nullptr || locs.find(bb) != locs.end()) {
    return;
  }

  locs[bb] = instructions.size();
  instructions.insert(instructions.end(), bb->instructions.begin(),
                      bb->instructions.end());
  if (bb->falseTarget) {
    instructions.push_back(
        Instruction(Operation::Not, std::optional<int32_t>()));
...
void fixJumps(InstructionList& instructions,
              std::map<BasicBlock*, int32_t>& locs,

...
void Parser::ErrorOut(antlr4::Token* token, string message) {
  cout << "Unexpected Symbol (" << token->getLine() << ","
       << token->getCharPositionInLine() << "): " << token->getText() << endl
       << token->getType() << endl;
...
void Parser::consume(size_t token_type) {
  if (current_token->getType() == token_type) {
    ...
    ErrorOut(current_token,
             "expected wrong type(" + to_string(token_type) + ") for consume");
  }
}
...
bool Parser::failed() const { return errored; }
...
shared_ptr<Block> Parser::ParseBlock() {
  ...
  while (!ended() && !failed() &&
         current_token->getType() != MITScript::CBRACK_KEY) {
    auto stmt = ParseStmt();
...
shared_ptr<Expr> Parser::ParseExpr() { return ParsePredicate(); }

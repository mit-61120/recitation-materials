...
void birthday2(const User &user) {
    user.age++;
}
/*
...
int main() {
    ...
    std::cout << users2[0].age << std::endl;
    users2[0].birthday();
    std::cout << users2[0].age << std::endl;

    std::cout << users2[1].age << std::endl;
    birthday2(users2[1]);
    std::cout << users2[1].age << std::endl;

    // TODO find all users whose names start with L or M (inclusive) and output them
    std::vector<User> lmUsers;
    std::copy_if(users.begin(), users.end(), std::back_inserter(lmUsers), [users2](const User &u) -> bool {

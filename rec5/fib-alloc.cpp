...
// static and in separate class so that all template instantiations of counting_allocator use the same counter
class alloc_counter {
public:
    static inline unsigned long long total_memory;
};

/// counting allocator wraps a std::allocator to track allocations and count the current allocated size.
template<typename T>
...
int main(int argc, const char *argv[]) {
    ...
    std::cout << "Memory used: " << alloc_counter::total_memory << "B" << std::endl;
